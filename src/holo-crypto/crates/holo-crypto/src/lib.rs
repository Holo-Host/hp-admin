...
use base64::encode;
use ed25519_dalek::{Keypair as EdKeypair, SecretKey, SECRET_KEY_LENGTH};
...
impl KeyPair {
    ...
    pub fn new(passphrase: &str, salt: &str) -> Result<Self, HoloCryptoError> {
        ...
        match salt_bytes.len() {
            0..=ARGON2_MAX_SALT_LEN if salt_bytes.len() < ARGON2_MAX_SALT_LEN => Err(
