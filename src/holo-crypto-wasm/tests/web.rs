...
fn admin_can_derive_key_and_sign() {
    let akp = AdminKeyPair::new("some@email.test", "p4ssw0rd", "HCA01adefasdtbeodbgahfawnf")
...
fn webuser_can_derive_key_and_sign() {
    let akp = WebUserKeyPair::new("some@email.test", "p4ssw0rd", "saltysalt", "QmDnaHash...")
        .expect("Could not generate keypair");
...
fn webuser_will_error_if_salt_too_short() {
    assert!(WebUserKeyPair::new("some@email.test", "pwd", "HCA01..", "QmDnaHash...").is_err())
}
