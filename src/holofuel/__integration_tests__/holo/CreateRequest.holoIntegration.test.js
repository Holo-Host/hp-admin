...
orchestrator.registerScenario('Tryorama Runs Create Request e2e', async scenario => {
  ...
  describe('Create Request Flow e2e', () => {
    it('All endpoints work e2e with DNA', async () => {
      ...
      hostedAgentDetails.id = `QmWvWtw1P2nduQqk8uKpfhAeUd72bSU2gFH7TLrQm1Bbfw::${copiedText}-${DNA_INSTANCE}`
      ...
      console.log('end--------', conterpartyAgentAddress)
      ...
      let counter = 0
      while (counter < 10) {
        let pending_list = await counterpartyAgentInstance.call('holofuel', 'transactions', 'list_pending', {});
        console.log('Checking:', pending_list)
        console.log('Checking:', pending_list.Ok.requests)
        if (pending_list.Ok.requests.length == 1) {
          pass = true
          break
        }
        await wait(10000)
